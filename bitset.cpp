...
class Sut {
	...
	size_t age{};
public:
	Sut(const char* name) : m_name("name"), age{0}{

	}
	//20 / 20 / 20 / 40
	//15 / 12 / 17 / 21 = 27 + 38 = 65.
...
int main() {
	char* name = "Nonthachai";
	::display(name);
	int osks[5]{10,20,30,40,3};
	cout << "[ " << osks[5]++ << "]\n";
	std::vector<double> tmp4(10);
	std::vector<double> tmp5(10,3);
	std::vector<double> tmp53;
	auto red =	for_each(tmp4.begin(),tmp4.end(),[&](double& val){
		val += tmp4.front();
		cout << "value" << val << "\n";
	});

	std::copy_if(tmp4.begin(),tmp4.end(),tmp5.begin(),[](double& val){
		if(val == 3){
			return true;
		}
		return false;
	});
	std::sort(tmp5.begin(),tmp5.end(),[](double sort1, double sor2){
		if(sort1 > sor2){
			return sort1;
		}
	});
